...
For TLS certificate generation, customize the CFSSL configuration files in the `services/cfssl/` directory. Generated certificates are output to `services/certs/` (gitignored).

## Root CA Configuration (`services/cfssl/ca.json`)
...
```bash
# Generate the full certificate chain (CA → Intermediate → Server) in one command
task certs:all
...
The `task certs:all` command automates the following steps:
...
All generated certificates are written to `services/certs/`, which is gitignored.
